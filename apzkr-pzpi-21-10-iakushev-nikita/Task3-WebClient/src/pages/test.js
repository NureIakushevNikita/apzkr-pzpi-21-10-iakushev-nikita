...
const Test = () => {
    ...
    const [applyDiscount, setApplyDiscount] = useState(false); // Новое состояние для применения скидки
    ...
    useEffect(() => {
        const fetchStoreAndMall = async () => {
            if (cartItems.length > 0) {
                ...
                try {
                    const storeResponse = await axios.get(`http://localhost:3000/store/${storeId}`);
                    const mallId = storeResponse.data.mall_id;
                    const mallResponse = await axios.get(`http://localhost:3000/mall/${mallId}`);
    ...
    useEffect(() => {
        const fetchDiscounts = async () => {
            try {
                ...
                const response = await axios.get(`http://localhost:3000/discount/${userId}`);
    ...
    const handleOrder = async () => {
        try {
            ...
            const newOrder = await axios.post('http://localhost:3000/order', {
                user_id,
                address,
            ...
            if (applyDiscount) {
                const highestPriceOrderDetails = cartItems.reduce((maxItem, item) => {
                    return item.price * item.num > maxItem.price * maxItem.num ? item : maxItem;
                });

                await axios.post(`http://localhost:3000/applyDiscount/${discounts.id}/${highestPriceOrderDetails.id}`);
            }
    ...
    <div className="order-discounts">
        ...
        {discounts.length === 0 ? (
            ...
            <ul>
                ...
                <button className="order-button" onClick={() => setApplyDiscount(!applyDiscount)}>Apply</button> {/* Добавлено управление состоянием applyDiscount */}
            </ul>
        )}
    </div>
</div>
